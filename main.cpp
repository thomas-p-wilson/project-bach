...
void note(ofstream& f, double frequence, double seconds);
...
int main()
{
  ...
  write_word( f, (sample_rate * bit_rate * channels) / 8, 4 );  // (Sample Rate * BitsPerSample * Channels) / 8
  write_word( f, 4, 2 );  // data block size (size of two integer samples, one for each channel, in bytes)
  write_word( f, bit_rate, 2 );  // number of bits per sample (use a multiple of 8)
